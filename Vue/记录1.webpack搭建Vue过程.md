...
config = {
    ...
    plugins: [
        ...
        new HTMLPlugin()//如果想要自行指定index.html渲染，在此处添加参数，详情请参考额外记录
    ]
}
//开发模式，易于调试
if (isDev) {
...
```shell
cnpm i style-loader url-loader file-loader html-webpack-plugin webpack-dev-server cross-env
```
...
在`webpack.config.js`的第52行关于html-webpack-plugin配置中，如果想要指定index.js中的div渲染绑定到指定的HTML文件中，可以添加参数。
